...
void mpu6050_read(uint8_t reg_addr, uint8_t *data, size_t len) {
    if (len == 0) return;
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    i2c_master_write_byte(cmd, (MPU6050_ADDR << 1) | I2C_MASTER_WRITE, true);
...
void mouse_logic_task(void *pvParameters)
{
    ...
    while(1) {
        ...
        // Логіка X (Roll)
        if (roll > deadzone) mouse_x = (int8_t)((roll - deadzone) * sensitivity);
        else if (roll < -deadzone) mouse_x = (int8_t)((roll + deadzone) * sensitivity);
        
        // Логіка Y (Pitch) - інвертуємо за потребою
        if (pitch > deadzone) mouse_y = (int8_t)((pitch - deadzone) * sensitivity);
        else if (pitch < -deadzone) mouse_y = (int8_t)((pitch + deadzone) * sensitivity);
...
void app_main(void)
{
    ...
    //esp_hidd_dev_init(&hidd_config, ESP_HID_TRANSPORT_BLE, bt_hidd_event_callback, &hid_conn_id);
        // ESP_ERROR_CHECK(
        //     esp_hidd_dev_init(&hidd_config, ESP_HID_TRANSPORT_BLE, bt_hidd_event_callback, &s_bt_hid_param.hid_dev));
